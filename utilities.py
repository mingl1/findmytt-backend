...
def videos_to_check(videos, location, check_failures):
    ...
    if check_failures:
        return [
            v
            for v in videos
            if video_url_to_id(v.get("Link", v.get("VideoLink"))) in failed_ids
        ]
    else:
        return [
            v
            for v in videos
            if video_url_to_id(v.get("Link", v.get("VideoLink"))) not in existing_ids
...
def remove_failure(tiktok_id, location):
    ...
    with open(file_location) as f:
        ...
        with open(file_location, "w") as f:
            json.dump(
                {k: v for k, v in failures.items() if k != tiktok_id}, f, indent=4
            )
